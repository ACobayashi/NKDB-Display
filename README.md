...
3. 可在“新增学生成员”中添加一名学生，说明系统具备基础数据维护能力。
4. 在“活动报名办理”选择学生和活动，点击“提交报名”，说明新增报名由触发器 `trg_before_registration_insert` 校验。
5. 再尝试选择满员活动或重复报名组合，说明触发器阻止非法插入。
6. 在“活动积分结算”选择五月数据库实践工作坊，点击“执行结算”，说明存储过程 `sp_finish_activity` 批量更新积分、报名状态和活动状态。
7. 再次结算同一个活动，说明存储过程阻止重复加分。
8. 在“活动数据删除”选择活动，点击“删除活动”，说明后端使用 `BEGIN / COMMIT / ROLLBACK` 保证活动、报名和通知一起删除。
