...
-- 如果视图已经存在，先删除，避免重复导入时报错。
DROP VIEW IF EXISTS v_activity_summary;

-- v_activity_summary：活动总览视图。
-- 作用：把活动、分类、主办方、场地、报名统计整合成一个查询结果。
-- 前端活动总览页面直接查询该视图，不需要在后端重复编写复杂多表连接。
CREATE VIEW v_activity_summary AS
SELECT
  -- 活动基础属性，来自 activities 表。
  a.activity_id,
  a.title,
  -- 活动分类名称，来自 activity_categories 表。
  ac.category_name,
  -- 主办方名称，来自 clubs 表。
  c.club_name,
  -- 场地名称，来自 venues 表。
  v.venue_name,
  -- venue_location 是前端表格展示字段，这里直接使用 venue_name。
...
-- activities.category_id = activity_categories.category_id。
JOIN activity_categories AS ac ON a.category_id = ac.category_id
-- activities.club_id = clubs.club_id。
JOIN clubs AS c ON a.club_id = c.club_id
-- activities.venue_id = venues.venue_id。
