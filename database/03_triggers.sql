...
-- 如果触发器已经存在，先删除，避免重复导入时报错。
DROP TRIGGER IF EXISTS trg_before_registration_insert;

DELIMITER //

-- trg_before_registration_insert：报名添加控制触发器。
-- 触发时机：向 registrations 表 INSERT 之前执行。
-- 作用：统一检查活动、用户、状态、截止时间、重复报名和名额。
CREATE TRIGGER trg_before_registration_insert
BEFORE INSERT ON registrations
FOR EACH ROW
BEGIN
  -- v_activity_exists：活动是否存在。
  DECLARE v_activity_exists INT DEFAULT 0;
  -- v_capacity：活动容量。
  DECLARE v_capacity INT DEFAULT 0;
  -- v_registered_count：当前已占用名额的人数。
  DECLARE v_registered_count INT DEFAULT 0;
  -- v_duplicate_count：该学生是否已经报名过同一活动。
  DECLARE v_duplicate_count INT DEFAULT 0;
  -- v_user_exists：用户是否存在。
  DECLARE v_user_exists INT DEFAULT 0;
  -- v_deadline：活动报名截止时间。
  DECLARE v_deadline DATETIME;
  -- v_status：活动状态，只有 open 才能报名。
  DECLARE v_status VARCHAR(20);
  -- v_user_role：用户身份，只有 student 可以报名。
  DECLARE v_user_role VARCHAR(20);

  -- 第一步：检查 NEW.activity_id 对应的活动是否存在。
  SELECT COUNT(*) INTO v_activity_exists
  FROM activities
  WHERE activity_id = NEW.activity_id;

  -- 如果活动不存在，直接抛出错误，阻止 INSERT。
  ...
  -- 第三步：检查活动状态，只有 open 状态允许报名。
  IF v_status <> 'open' THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = '报名失败：活动不是开放报名状态';
  END IF;

  -- 第四步：检查当前时间是否已经超过报名截止时间。
  IF v_deadline < NOW() THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = '报名失败：活动报名已截止';
  END IF;

  -- 第五步：检查重复报名，已取消的记录不计入重复。
