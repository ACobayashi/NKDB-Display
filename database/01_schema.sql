-- 重新导入时先删除旧库，保证演示数据和结构从干净状态开始。
DROP DATABASE IF EXISTS campus_activity_db;

-- 创建校园活动报名数据库，utf8mb4 可以正常保存中文。
...
CREATE TABLE users (
  ...
  -- role 区分 student 和 admin，触发器会要求只有 student 能报名。
  role ENUM('student', 'admin') NOT NULL DEFAULT 'student',
  -- major、class_name 保存学生专业和班级。
  major VARCHAR(80),
...
CREATE TABLE clubs (
  ...
  -- contact_name、contact_phone 保存联系人信息。
...
CREATE TABLE venues (
  ...
  -- venue_name 是前端下拉框展示的场地名称。
  venue_name VARCHAR(80) NOT NULL,
  -- building 和 room 描述场地所在建筑和房间/区域。
...
CREATE TABLE activity_categories (
  ...
  -- category_name 是分类名称，唯一约束避免重复分类。
...
CREATE TABLE activities (
  ...
  -- start_time、end_time、registration_deadline 控制活动时间和报名截止时间。
  start_time DATETIME NOT NULL,
  end_time DATETIME NOT NULL,
  registration_deadline DATETIME NOT NULL,
  -- capacity 是活动名额，points 是活动基础积分。
  capacity INT NOT NULL,
  points INT NOT NULL DEFAULT 1,
  -- status 表示活动状态，报名触发器和结算存储过程都会检查该字段。
  status ENUM('draft', 'open', 'closed', 'finished', 'cancelled') NOT NULL DEFAULT 'open',
  -- created_by 关联 users.user_id，表示活动创建者，一般为管理员。
  created_by INT NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  -- 外键：activities.category_id -> activity_categories.category_id。
  CONSTRAINT fk_activities_category
    FOREIGN KEY (category_id) REFERENCES activity_categories (category_id),
  -- 外键：activities.club_id -> clubs.club_id。
  CONSTRAINT fk_activities_club
    FOREIGN KEY (club_id) REFERENCES clubs (club_id),
  -- 外键：activities.venue_id -> venues.venue_id。
  CONSTRAINT fk_activities_venue
    FOREIGN KEY (venue_id) REFERENCES venues (venue_id),
  -- 外键：activities.created_by -> users.user_id。
  CONSTRAINT fk_activities_created_by
    FOREIGN KEY (created_by) REFERENCES users (user_id),
  -- 基础检查约束：名额为正、积分非负、结束时间晚于开始时间。
  CHECK (capacity > 0),
  CHECK (points >= 0),
...
CREATE TABLE registrations (
  -- registration_id 是报名记录主键。
  registration_id INT PRIMARY KEY AUTO_INCREMENT,
  -- activity_id 关联 activities，表示报名的是哪一个活动。
  activity_id INT NOT NULL,
  -- user_id 关联 users，表示哪一个学生报名。
  user_id INT NOT NULL,
  -- status 表示报名状态：已报名、已签到、已完成、已取消。
  status ENUM('registered', 'checked_in', 'completed', 'cancelled') NOT NULL DEFAULT 'registered',
  -- registered_at 和 checked_in_at 保存报名时间与签到时间。
...
CREATE TABLE activity_notices (
  ...
  -- activity_id 关联 activities，表示通知属于哪个活动。
  activity_id INT NOT NULL,
  -- title 和 content 保存通知标题与正文。
  title VARCHAR(120) NOT NULL,
  content TEXT NOT NULL,
  -- published_at 记录通知发布时间。
...
-- 索引用于提高常用查询效率：活动状态、活动时间、报名按活动/用户查询。
